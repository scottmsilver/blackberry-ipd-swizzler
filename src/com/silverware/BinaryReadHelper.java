...
class BinaryReadHelper {
  ...
  /*
   * Number of bytes read since last call to startCountingBytes or since construction. 
   */
  private int counter;
  ...
  // length includes \0
  String getNullTerminatedString(String fieldName, int length) throws IOException {
  ...
  private void debugOutput(String fieldName, String fieldValue, int length) {
    //System.out.format("field: %s at %x: %s\n", fieldName, offset - length, fieldValue);
  }
